#!/usr/bin/env bash
# Dedicated EIR suction-camera ingress.  It preserves source messages and
# keeps the existing CAM1/CAM3/CAM4/FLIR ingress process group untouched.
set -euo pipefail
...
exec "${PERCEPTION_PYTHON:-python3}" "${BIN}" --ros-args \
  -r __node:=perception_ingress_suction \
  -p camera:=suction
